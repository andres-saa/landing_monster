<template>
  <div style="max-width: 800px;background-color: var(--primary-color);border-radius: 1rem;"
    ...
    <div class="header   " style=" justify-content: space-between; ;z-index:99;">


      <p class=" mayuscula  nombre col-9 text-6xl lg:text-7xl p-0  text-left text-white"
        style="color:black;line-height: 4rem; font-weight: bold">
        {{
    ...
    <div class=" col-12 p-0  ">
      ...
      <div class="col-12 p-0 "
        style="display: flex;align-items: center; border-radius: 0.5rem;flex-direction: column;gap: 2rem; ">




        <Button rounded class="text-xl md:text-5xl" @click="addToCart(store.currentProduct)" label="REALIZAR PEDIDO"
          style=" pointer-events: all;border: none; color:white;background-color: black; width:100%;padding: 0.5rem 3rem; font-weight: bold;">
        </Button>
      ...
      <div v-show="!see" class="col-12  p-0 mt-0 shadow-5"
        style="display: flex;align-items: center; max-height: 45rem;border-radius: 0.5rem; ">
      ...
      <div class="   col-12 p-0 my-5" style="  " :class="!isSmallScreen ? 'scroll' : 'static'">
        <p class="col-12 text-4xl p-0" style="font-weight: bold;color: black; ">
          DESCRIPCION

        </p>


        <p class="col-12 text-2xl md:text-4xl p-0" style="color: black;text-transform:capitalize">
        ...
        <div style="color: black;">
          ...
          <div v-for="grupo in adicionales" :key="grupo.category">
            <div class="mb-2">
              <span class="mb-2 text-center text-4xl">
                <b>{{ grupo.category }}</b>
              </span>
              <div class="mt-2">
                <div v-for="item in grupo.items" :key="item.aditional_item_instance_id"
                  ...
                  <Checkbox class="my-2" style="color: black;" :binary="true" v-model="item.checked"
                    @change="() => handleAdditionChange(item, grupo.category)" />
                  <div style="display: flex; width: 100%; gap: 1rem; justify-content: space-between;">
                    <span class="text-xl md:text-4xl adicion text-black" style="text-transform: lowercase;">{{
                    ...
                    <span v-if="item.checked" style="display: flex; align-items: center;">
                      ...
                      <span v-if="item.aditional_item_price > 0" class="pl-2 py-2 text-xl">
                        <b>{{ formatoPesosColombianos(item.aditional_item_price *
                          selectedAdditions[item.aditional_item_instance_id]?.quantity) }}</b>
                      </span>

                      <Button @click="decrement(item)" class="ml-2"
                        style="width: 2rem; height: 1.5rem;border: none;background-color: black;"
                        icon="pi pi-minus"></Button>
                      <InputText :modelValue="selectedAdditions[item.aditional_item_instance_id]?.quantity" readonly
                        style="width: 2rem;border: none; height: 1.5rem;color: black;" class="p-0 text-center" />
                      <Button @click="increment(item)" severity="danger"
                        style="width: 2rem;height: 1.5rem; border: none; none;background-color: black;"
                        icon="pi pi-plus"></Button>
                    </span>



                    <span v-else-if="item.aditional_item_price > 0" class="pl-2 py-2 text-2xl">
                      <b>{{ formatoPesosColombianos(item.aditional_item_price) }}
                      </b>
                    </span>
        ...
        <Button rounded class="text-xl md:text-5xl" @click="addToCart(store.currentProduct)" label="REALIZAR PEDIDO"
          style=" pointer-events: all; color:white;background-color: black; width:100%;padding: 0.5rem 3rem; font-weight: bold;">
        </Button>
...
import { onMounted, onBeforeUnmount, watch, onBeforeMount } from 'vue'
